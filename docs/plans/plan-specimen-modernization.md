...
- [x] ~~No dedicated secrets-management view; secrets appear to be managed only as flagged rows inside the Environments editor, which under-signposts the "vault" concept the first-use flow sets up.~~ **Fixed** — new `SecretsManagerComponent` (`src/app/components/secrets/`), a dialog matching the Local Bridge settings dialog's visual/interaction pattern, lists every secret across every environment, supports lock/unlock-aware reveal, rename, delete, and a "locate" chip per usage that hands off to the existing `VariableFocusService` flow. Reuses `SecretsService`/`SecretCryptoService`/`SecretsRepository` for all crypto/persistence; added the missing `listAll()`/`renameSecret()`/`deleteSecret()` aggregate methods to `SecretsRepository` since none existed. Registered in the command palette plus a persistent toolbar icon button.
- [x] ~~No Settings surface anywhere (theme toggle exists, but no reset-all-data, keyboard shortcuts reference, or preferences screen) — partially addressed: Reset All Data and Local Bridge settings are now both reachable from the command palette, but there's still no dedicated Settings screen.~~ **Fixed** — new `SettingsComponent` (`src/app/components/settings/`) consolidates theme toggle, environments export/import (reusing `environment-io.util.ts`'s existing serialize/validate functions as-is), Reset All Data and Local Bridge settings (still owned/executed by `AppShellComponent`, requested via outputs), and a keyboard-shortcuts reference (fixed global chords + a live list of every currently-registered command-palette action). Registered in the command palette plus a persistent toolbar icon button.
- [x] ~~Command palette (⌘K) currently has exactly one registered command ("New Collection") — the UI for it is done, the content isn't.~~ **Fixed** — registered New Request, Send Request, Focus Address Bar, theme toggle, open History, lock/unlock secrets, Local Bridge settings, and Reset All Data, sourced from `AppShellComponent` (the sidebar palette had no access to those) via a new `[externalActions]` input on `CollectionsSidebarComponent`. Manage Secrets and Settings added to the same list in Phase 3.
- [x] ~~Fixed-width centered layout leaves large amounts of dead canvas at 1024px–1440px+ where every competitor uses a resizable multi-pane split.~~ **Fixed** — desktop composer/response layout now uses PrimeNG `p-splitter`, split ratio persisted via the splitter's own `stateStorage="local"`/`stateKey="wayfarer:composer-split"`. The desktop-tabs/mobile-accordion split now also mutually excludes via `@if`/`@else` on the `isMobile` signal instead of both always being mounted with one CSS-hidden.
- [x] ~~Disabled "Copy as cURL" button renders as an empty box, easily mistaken for broken rather than disabled.~~ **Fixed** — wrapped in a non-disabled span carrying a `pTooltip` ("Enter a URL first to copy as cURL") and a real `not-allowed` cursor (disabled native `<button>`s ignore author `cursor` in most browsers), plus the existing `.send-btn`-style reduced-opacity disabled convention. Surfaced (and fixed) a real pre-existing bug while wiring this up: `ApiParamsComponent` had no `styleUrls` at all, so `api-params.component.css` — including its long-standing address-bar/method-select rules — was dead code.
...
| Task | Detail | Status |
|---|---|---|
| Resizable multi-pane layout | Composer/response split like Postman/Insomnia/Bruno; stop capping content to a fixed-width centered card | **Done** — PrimeNG `p-splitter` at desktop widths, split ratio persisted via `stateKey="wayfarer:composer-split"`. Desktop-tabs/mobile-accordion are now mutually exclusive (`@if`/`@else` on `isMobile`) instead of both permanently mounted with one CSS-hidden. |
| Rebuild the mobile composer | Accordion or segmented-control pattern at narrow widths instead of collapsing all four tabs into one unlabeled stacked column; fix Monaco initializing inside zero-width containers | **Done** — mobile accordion is now strictly single-panel-at-a-time (dropped `[multiple]="true"`). Monaco (`script-editor`/`json-editor`) waits for a resolved non-zero width via a shared `waitForNonZeroWidth()` ResizeObserver helper before calling `monaco.editor.create()`. Separately, reproduced and fixed a real `@defer (on viewport)` reliability gap under rapid viewport/tab churn — see the render-glitch row below. |
...
| Build a dedicated Secrets management view | Beyond the per-variable lock icon buried in the Environments editor | **Done** — `SecretsManagerComponent`, a Local-Bridge-dialog-styled `p-dialog` listing every secret across every environment with reveal/rename/delete/locate, reusing `SecretsService`/`SecretCryptoService`/`SecretsRepository` for all crypto/persistence. Registered in the command palette + a toolbar icon button. |
| Build a Settings surface | Theme, data export/import, reset-all-data, keyboard shortcuts reference | **Done** — `SettingsComponent` consolidates all of the above; Reset All Data and Local Bridge settings still owned/executed by `AppShellComponent`, requested via outputs; environments export/import reuses `environment-io.util.ts` as-is; shortcuts reference includes a live list of registered palette actions. Registered in the command palette + a toolbar icon button. |
| Fix the small polish bugs from Part D | Disabled-button visibility, intermittent render glitch on tab/viewport transitions | **Done** — cURL button gets a real disabled state (opacity + not-allowed cursor via a wrapper span + explanatory tooltip). The render glitch was reproduced concretely with an instrumented Playwright script: `@defer (on viewport)`'s IntersectionObserver trigger can be missed entirely under rapid structural churn, permanently stranding a JSON editor on "Loading editor…" with a real non-zero width. Fixed with an `on timer(400ms)` fallback trigger (OR semantics) on both Monaco-hosting `@defer` blocks; re-verified clean across multiple repro runs. Regression coverage in `e2e/layout.spec.ts`. |
| Complete the command palette | Register the toolbar actions that already exist as functions (theme toggle, history, sidebar toggle, new environment, send) — mostly a registration exercise, not new UI | **Done** — Manage Secrets and Settings added alongside the existing Phase 3-prior entries. |
| Accessibility pass | ARIA roles/labels across all interactive components; focus trap + restore for every dialog/drawer; add `axe-core` (or Pa11y) as a CI gate targeting 0 critical/serious violations on primary flows | Mostly done (prior session) — this pass added coverage for the Save-to-Collection dialog and command palette, and fixed a real Send-button contrast failure the existing gate had been inadvertently masking. This session's UI work stayed inside that gate: fixed a real tooltip color-contrast bug the new cURL tooltip surfaced (`--label-secondary` on `--canvas-overlay` measured 4.27:1; switched to `--label-secondary-on-fill`), and documented/excluded one confirmed-upstream PrimeNG Splitter ARIA-attribute-placement bug following the file's existing exclusion pattern. |
| Motion pass | Use the existing Obsidian design system's spring-easing tokens for tab switches, response arrival, dialog open/close — deliberate, not decorative | **Done** — tab switches (composer, response viewer, environments editor, and the mobile accordion) now animate via a shared `.p-tabpanel:not([hidden])` rule and an `[transitionOptions]` binding using this app's own duration/easing tokens; response arrival upgraded to a dedicated `.animate-response-arrive` (fade-up + `--ease-spring`); dialog open/close was already covered globally (`.p-dialog { animation: dialog-appear }` + `provideAnimations()`) and the new Secrets/Settings dialogs inherit it for free. Added the app's first `prefers-reduced-motion` convention (a blanket CSS override in `animations.css`, plus a `prefersReducedMotion()` helper for the one Angular-animations-driven surface — Accordion — that a CSS media query can't reach). |
